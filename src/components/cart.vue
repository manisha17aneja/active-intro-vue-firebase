                    ...
                    <td class="product-discription-td">
                      <h6>
                        <strong
                          ><router-link
                            :to="{ path: '/subscriptions/' + subscription.id }"
                            >{{ subscription.title }}</router-link
                          ></strong
                        >
                      </h6>
                      <p>{{ subscription.description }}</p>
                    </td>
                    <td class="empty-th" title="Amount">
                      AED {{ subscription.vat }}
                    </td>
                    <td class="empty-th text-center" title="Amount">
                      <i
                        class="fas fa-times"
                        @click="deleteItem(subscription.cart_id)"
                      ></i>
                    </td>
                  </tr>
                </tbody>
        ...
        <section class="payment-section" v-if="this.subscriptions.length">
          <div class="payment">
            ...
            <div class="stripe" v-else>
              <router-link to="/signin" class="btn btn-primary btn-pay-now"
                >Pay Now</router-link
              >
            </div>
          </div>
...
export default {
  ...
  data() {
    this.pulishableKey =
      "pk_test_51ITRnMH7OARwqCbCLC1bloHLpobVzLqirExLcUsFAbIle8t8OhB8KrBTAzTLjPOn9an6kM6WB98wuR9ulRs2jy4w00TmtbEpLz";
    return {
      subscriptions: [],
      category: "",
      loading: false,
  ...
  methods: {
    ...
    async tokenCreated(token) {
      ...
      axios.post("http://159.65.231.72/api/charge", data).then((response) => {
          ...
          querySnapshot.forEach((docs) => {
            ...
            db.collection("subscriptions")
              ...
              .then((doc) => {
                ...
                var discountPrice =
                  doc.data().price -
                  (doc.data().price * doc.data().discount) / 100;
                var vat =
                  discountPrice + (discountPrice * doc.data().vat) / 100;
